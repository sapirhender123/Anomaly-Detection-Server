//
// Created by s on 27/11/2020.
//

#ifndef BUILDSTONE1_MINCIRCLE3_H
#define BUILDSTONE1_MINCIRCLE3_H
...
/// function that return the center of the circle that created by two points
Point findCenter(Point point1, Point point2) {
    ...
    return {(point2.y * powP1 - point1.y * powP2) / (2 * vecMult), (point1.x * powP2 - point2.x * powP1) / (2 * vecMult)};
}
...
Circle circle_from(const Point &A, const Point &B, const Point &C) {
    ...
    Point point1 = subtractTwoPoints(B, A);
    Point point1 = subtractTwoPoints(B, A);
    Point I = findCenter(point1, C.x - A.x, C.y - A.y);
...
#endif//BUILDSTONE1_MINCIRCLE3_H
