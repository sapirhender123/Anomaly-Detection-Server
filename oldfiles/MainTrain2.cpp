...
int main() {
    ...
    for (size_t i = 0; i < N; i++) ps_copy[i] = new Point(ps[i]->x, ps[i]->y);
    //    ps_copy[0] = new Point(1,0);
    //    ps_copy[1] = new Point(0,1);
    //    ps_copy[2] = new Point(0,-1);
    //    ps_copy[3] = new Point(-1,0);
    ...
    cout << "The center is: (" << c.center.x << "," << c.center.y << ")" << endl;
    cout << "The radius suppose to be: " << (int) R << endl;

    if ((int) c.radius > (int) R) cout << "you need to find a minimal radius (-40)" << endl;
    ...
    if (!covered) cout << "all points should be covered (-45)" << endl;
